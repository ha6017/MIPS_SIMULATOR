...
compiler::compiler(std::string binaryfile) : mem(binaryfile), reg() //the constructor of class compiler initialises the object mem of class memory
...
void compiler::runItype(uint32_t instruction)
{
  ...
  signExtImmediate = int32_t(immediate << 16) >> 16; // int32_t type
  instr=instruction;

  op1s = regs.read(rs);//int type
  op2s = regs.read(rt);
  op1 = regs.read(rs);//uint type
  op2 = regs.read(rt);
  ...
  switch(opcode)
  {
    ...
    case 13:
      //ORI
      regs.write(rt,(op1|(int32_t(immediate)&&0xFFFF))); // zero extended
      break;
    case 10:
      //SLTI
    ...
    case 41:
      ...
      mem.store_halfword_to_memory((regs.read(rs)+signExtImmediate), val);
      break;
    case 43:
      //SW
